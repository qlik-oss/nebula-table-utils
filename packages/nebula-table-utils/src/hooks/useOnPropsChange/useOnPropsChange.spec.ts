...
describe('useOnPropsChange', () => {
  ...
  test('should not run callback function when a reference dependency does not changes', () => {
    const callback = jest.fn();
    const fakeLayout = { qHyperCube: { qSize: { qcy: 1 } } };
  ...
  test('should not run callback function when a primitive dependency does not changes', () => {
    const callback = jest.fn();
